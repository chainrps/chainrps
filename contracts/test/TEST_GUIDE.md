...
3. 编译 RPSGame.sol 和 MockERC20.sol
...
| 函数 | 功能 | 调用方 |
|------|------|--------|
| `createMatch(amount, token)` | 创建对局 | 任意用户 |
| `joinMatch(gameId)` | 加入对局 | 任意用户 |
| `submitCommit(gameId, commit)` | 提交哈希承诺 | 对局玩家 |
| `revealChoice(gameId, choice, salt)` | 揭晓出拳 | 对局玩家 |
| `claimTimeout(gameId)` | 超时索赔 | 对局玩家 |
| `handleDraw(gameId)` | 平局退款 | 对局玩家 |
...
| `setTokenSupport(token, supported)` | 添加/移除支持代币 |
...
| `TimeoutClaimed(gameId, claimer)` | 超时索赔 |
| `DrawHandled(gameId)` | 平局处理完成 |
...
python deploy.py --contract RPSGame --network amoy \
...
**操作流程：**
1. 玩家A 先调用 `approve(rpsGameAddress, betAmount)` 授权
...
**操作流程：**
1. 玩家B 调用 `approve(rpsGameAddress, betAmount)` 授权
...
- [ ] `isDraw` = true
- [ ] `status` = Finished (3)
- [ ] 玩家A调用 handleDraw 后，收到全额退款
- [ ] 玩家B调用 handleDraw 后，收到全额退款
- [ ] `DrawHandled` 事件在双方都退款后触发
- [ ] **平局无手续费**
...
1. 玩家A 提交承诺，玩家B 不提交
2. 等待超过 66 秒
3. 玩家A 调用 `claimTimeout(gameId)`

**预期：**
- [ ] `TimeoutClaimed` 事件触发
- [ ] 玩家A 获胜，收到全部奖金（2 * betAmount - fee）
- [ ] feeCollector 收到手续费
...
**取消对局：**
- Owner 调用 `cancelMatch(gameId)`
- 验证：双方都收到全额退款
- 验证：`MatchCancelled` 事件
- 验证：状态变为 Cancelled (4)
- 注意：已结束的对局不能取消
...
| 3 | 平局退款 | 双方出拳相同→双方handleDraw | 各退本金，无手续费 | P0 |
| 4 | 提交阶段超时判负 | A提交，B不提交，超时后A索赔 | A胜 | P0 |
| 5 | 揭晓阶段超时判负 | 双方提交，A揭晓B不揭晓，超时A索赔 | A胜 | P0 |
| 6 | 哈希不一致作弊 | 揭晓时choice/salt与commit不匹配 | revert | P0 |
| 7 | Owner取消进行中对局 | Owner cancelMatch | 双方全额退款 | P1 |
| 8 | 合约暂停 | pause后尝试创建对局 | revert | P1 |
| 9 | 修改手续费率 | setFeeRate后新对局 | 按新费率扣费 | P1 |
| 10 | 修改开发者地址 | setDeveloperAddress | 手续费转新地址 | P1 |
| 11 | 跨对局重放攻击 | 用对局A的commit提交到对局B | 因包含address，哈希不匹配 | P0 |
| 12 | 重入攻击 | 在收款回调中重入 | nonReentrant防护 | P0 |
