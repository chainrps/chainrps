...
from rps_core.config import REVEAL_TIMEOUT
from rps_core.models import GameState, Choice, WSMessage
from rps_core.repository import (
    get_game_record,
    update_game_record,
    update_player_stats,
)
from rps_core.utils.helpers import calculate_deadline, deadline_to_iso
...
class GameManager:
    ...
    async def reveal_choice(self, game_id: int, player_address: str, choice: Choice, salt: str) -> dict:
        ...
        流程：
        1. 记录揭晓到数据库
        2. 通知对手 opponent_reveal

        注意：不做胜负判定，不做结算。
        胜负结果由 contract_service 从链上事件同步后通知。
        """
        game = get_game_record(game_id)
        if not game:
            return {"error": "Game not found"}
